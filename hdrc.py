...
def solve_poisson_equation_cuda(div_G, args):
    H, W = div_G.shape[:2]
    d_div_G = torch.from_numpy(np.asarray(div_G)).float().cuda()
    solve_args = []
    if args.method > 4:
        solve_args = [
            5, # v, w, f - cycle
            10, # pre/post smoothing iterations
            5, # coarsest level side length
            10000, # coarsest iterations
            100, # coarsest check frequency
            1e-4, # coarsest tolerance
            # 1.45 # omega/w for SOR
        ]
    result, iter_until_converge = solve(
        H, W, d_div_G, 
...
if __name__ == "__main__":
    ...
    arg_parser.add_argument("--check_frequency", type=int, default=30, help="Number of iterations to check if satisfy the tolerance")
